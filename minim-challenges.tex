...
\chapter{Minimality -- some observations and challenges}
...
\subparagraph*{Structure of this chapter.} This investigation will proceed in three parts. 
\begin{itemize}
  \item In Section~\ref{sec:min-dsa-props}, we will explore some of the properties of minimal DSAs and the surprising observation that minimal DSAs need not be derived from the canonical DFA.
  \item In Section~\ref{sec:complexity}, we will formalize the inherent difficulty of this problem by proving that DSA minimization is NP-complete. 
  \item Finally, in response to this complexity, in Section~\ref{sec:strong} we will introduce and analyze a well-behaved subclass, `Strongly Deterministic Suffix-reading Automata (sDSAs)', for which the desirable link to the canonical DFA is restored.
\end{itemize}
%marker
\vspace{.25cm}

\section{Some properties of minimal DSAs}
\label{sec:min-dsa-props}
...
\begin{lemma}
 ...
 \end{lemma}
\begin{proof}
  Figure~\ref{fig:no-canonical} illustrates two DSAs, both of size $8$ ($3$ states,
  ...
  least $2$. Therefore $|\Aa| \ge 2 + 4 + 2 = 8$, which is at least the
  size  of $|\Aa_1|$ and $|\Aa_2|$, and hence any DSA that accepts $aba$ through a single transition has total-size at least $8$.
  Now, suppose the accepting run of $\Aa$ on $aba$ has a
  ...
  $q' \xra{a} q_1$. 
  This discussion shows that the two automata $\Aa_2$ and $\Aa_1$,
  are indeed minimal.
\end{proof}

The next  observation
is that minimal DSAs will be well-formed (Definition~\ref{def:well-formed-dsa}): if there are two transitions
...
DSAs) can be derived from the corresponding tracking DFAs (see Definition~\ref{def:derived-dsa} for what it means to be ``derived'').
...
\paragraph*{A motivating example.} We describe another example of a strong DSA, motivated by a real-life situation. It is a simplified specification of an automotive
...
\begin{theorem}\label{thm:sdsa-from-min-dfa}
...
\paragraph*{Concluding remarks for Chapter~\ref{sec:minim-some-observ}.} This chapter considered the problem of DSA minimization, revealing a set of properties that differ significantly from classical automata theory. Our investigation yielded three primary contributions that deepen our understanding of the suffix-reading model.
