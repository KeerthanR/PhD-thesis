...
\chapter{Suffix-tracking sets -- deriving DSA from DFA}
...
\begin{description}
\item[Problem of interest.] how does one derive a correct and concise DSA from a standard representation like a DFA?
\end{description}
...
For the derivation to be sound, the collection of paths that will become the new DSA transitions must satisfy certain consistency requirements. An analogy can be drawn to ensuring a complex electronic circuit is functional: it is not enough for the individual wires to be well-made; they must also be connected correctly at their terminals to avoid ambiguity and short-circuits. Our derivation procedure rests on two such conditions.

First, we must ensure that the paths are \textbf{internally consistent}. This is the role of what we call a `suffix-compatibility' condition. It guarantees that as we trace a path through the suppressed part of the DFA, the logic of suffix-matching is maintained correctly at every single step. This is akin to checking that each wire in the circuit functions correctly along its entire length, faithfully transmitting signals.

Second, we must ensure the paths are consistent with each other at their \textbf{endpoints}—the selected macro-states. This is the purpose of what we call a `well-formedness' condition. It prevents ambiguity by forbidding situations where a path leading to a chosen macro-state could be mistaken for part of a different path leading to a suppressed state. This is analogous to checking that all wires are connected to the correct terminals in the circuit, ensuring that signals arrive at their intended destinations without interference.
...
with any DGA and suppress states). 

\paragraph*{Structure of this chapter.} Our procedure can be broken down
into two steps, which we will describe in the two sections of this chapter.
\begin{itemize}
\item In Section~\ref{sec:induced-dsa}, we start from a complete DFA, select a subset of states and build an induced DSA by connecting states using acyclic paths between them.
\item In Section~\ref{sec:removing-redundant}, we show how to remove some redundant transitions. In the end we obtain a potentially more concise DSA from the initial DFA.
\end{itemize}  
...
\section{Building an induced DSA}
\label{sec:induced-dsa}
...
one-letter extension. This condition on simple words, transfers to all
...
\section{Removing some redundant transitions.}
\label{sec:removing-redundant}
